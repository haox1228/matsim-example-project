...
public class VolumesAnalyzer implements LinkLeaveEventHandler, VehicleEntersTrafficEventHandler {
    //private final Logger log = Logger.getLogger(VolumesAnalyzer.class);
        ...
        for (int hour = 0; hour < 30; hour++) {
            ...
            if (hour >=24) {
                for (int i = 0; i < slotsPerHour; i++) {
                    volumes[hour-24] += volumesForLink[this.getTimeSlotIndex(time)];
